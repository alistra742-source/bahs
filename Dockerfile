...
# what comes back is the script. DeepSeek is the second model, called only in the "agent" and
...
#
# Five Python modules: bridge.py (the tokens, the config, one request and its stream, the two
# transports, and the sessions that keep one chat each), state.py (the health checks and the
# limits), luau.py (the toolbox, including the queue the executor polls), pow_solver.py (the
# proof of work chat.deepseek.com asks for) and server.py, which is the service on top: the mode,
# the turn, the tool rounds, the endpoints and the page.
